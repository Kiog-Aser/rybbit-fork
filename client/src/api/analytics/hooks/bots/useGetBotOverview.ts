...
export function useGetBotOverview({ site }: { site?: number | string }) {
  ...
  return useQuery({
    queryKey: ["bot-overview", time, site, botFilters, selectedLayer, selectedCategory, timezone],
    queryFn: () =>
      fetchBotOverview(site!, { ...params, layer: selectedLayer, category: selectedCategory }).then(data => ({ data })),
    staleTime: 60_000,
