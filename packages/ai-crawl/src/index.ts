 ...
 * API surface mirrors @datafast/ai-crawl so existing middleware can switch packages easily.
 */
...
export type AICrawlerCategory =
  (typeof AI_CRAWLER_CATEGORY)[keyof typeof AI_CRAWLER_CATEGORY];
...
export interface RybbitCrawlerTrackingConfig {
  /** Rybbit site id (string id from dashboard / script, e.g. d9411320640f). */
  siteId?: string;
  /** Alias for DataFast compatibility. */
  websiteId?: string;
  domain?: string;
  /** Defaults to https://analytics.milh.tech/api/ai-crawls */
  apiUrl?: string;
  enabled?: boolean;
...
function resolveSiteId(config: RybbitCrawlerTrackingConfig): string {
  return (config.siteId || config.websiteId || "").trim();
}

const DEFAULT_API_URL = "https://analytics.milh.tech/api/ai-crawls";
const DEFAULT_TIMEOUT_MS = 1500;
const DEFAULT_MAX_URL_LENGTH = 8192;
// AI_CRAWLER_CATEGORY exported above
const AI_CRAWLER_PROVIDERS = [
  {
    provider: "OpenAI",
    agents: [
...
const NORMALIZED_AGENTS = AI_CRAWLER_PROVIDERS.flatMap(
...
const AI_CRAWLER_CANDIDATE_ALIASES = [
  {
    provider: "OpenAI",
    agent: "OpenAI",
    category: AI_CRAWLER_CATEGORY.AI_CRAWLER,
...
const CRAWLER_FACING_EXACT_PATHS = /* @__PURE__ */ new Set([
  "/robots.txt",
  "/llms.txt",
  "/llms-full.txt"
]);
...
const STATIC_FETCH_DESTINATIONS = /* @__PURE__ */ new Set([
  "audio",
  "embed",
  "font",
  "image",
