...
export const importPlatforms = ["umami", "simple_analytics", "plausible", "rybbit_export"] as const;

export const importPlatformEnum = pgEnum("import_platform_enum", importPlatforms);

// Per-site Stripe revenue attribution (restricted API key + webhook)
export const siteStripeConnections = pgTable(
  "site_stripe_connections",
  {
    siteId: integer("site_id")
      .primaryKey()
      .references(() => sites.siteId, { onDelete: "cascade" }),
    restrictedKeyEncrypted: text("restricted_key_encrypted").notNull(),
  ...
  table => [
    foreignKey({
      columns: [table.siteId],
      foreignColumns: [sites.siteId],
      name: "site_stripe_connections_site_id_sites_site_id_fk",
    }),
  ]
);
